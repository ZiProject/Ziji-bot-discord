...
test("Prisma SQLite adapter exposes Mongoose-like model API", async () => {
	...
	try {
		...
		const user = await db.ZiUser.findOneAndUpdate(
			{ userID: "adapter-user" },
			{ $set: { name: "Adapter" }, $inc: { coin: 10 }, $addToSet: { thankedCookies: "cookie-1" } },
			{ upsert: true },
		);
		user.huntStats.common = { Cat: { count: 2 } };
		await user.save();

		const found = await db.ZiUser.findOne({ userID: "adapter-user", "huntStats.common.Cat.count": { $gte: 2 } });
...
test("client ready falls back to LocalDB when Prisma providers fail", async () => {
	...
	try {
		...
		const db = await initDatabase({ client: fakeClient });

		assert.ok(db, "initDatabase should return a database when Prisma providers fail");
		assert.strictEqual(db.provider, "localdb", "LocalDB should be the final fallback provider");
		assert.strictEqual(useHooks.get("db"), db, "The fallback DB should be registered in useHooks");
		assert.ok(db.ZiUser, "LocalDB should expose ZiUser");
		assert.ok(db.ZiGuild, "LocalDB should expose ZiGuild");
