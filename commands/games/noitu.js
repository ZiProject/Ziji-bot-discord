...
module.exports.execute = async ({ interaction }) => {
	...
	if (subcommand === "set-channel") {
		...
		return interaction.editReply({
			content: `✅ Kênh nối từ đã được đặt tại <#${target.id}>. Sử dụng lệnh \`/noitu start\` tại đây để bắt đầu trò chơi.`,
		});
	}
	...
	if (subcommand === "start") {
		...
		if (!GuildSetting?.noitu?.enabled || !GuildSetting.noitu.channel) {
			return interaction.editReply({
				content: "⚠️ Bạn chưa thiết lập kênh chơi nối từ. Hãy dùng lệnh `/noitu set-channel` trước.",
			});
		}
