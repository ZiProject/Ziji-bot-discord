...
const {
  ...
  buildHistoryEmbed, buildPityEmbed, buildStardustShopEmbed, buildCollectionEmbed,
...
const slashData = new SlashCommandBuilder()
  ...
  .addSubcommand(sub => sub
    .setName("banners")
    .setDescription("View all active gacha banners"))
  ...
  .addSubcommand(sub => sub
    ...
    .addStringOption(opt => opt
      .setName("banner")
      .setDescription("Banner to pull on")
      .setRequired(true)
  ...
  .addSubcommand(sub => sub
    ...
    .addStringOption(opt => opt
      .setName("banner")
      .setDescription("Filter by banner")
      .setRequired(false)
      .setAutocomplete(true)))

  .addSubcommand(sub => sub
    .setName("shop")
    .setDescription("Spend stardust on exclusive items"))

  .addSubcommand(sub => sub
    .setName("gems")
    .setDescription("Check your gem balance and how to earn more"));
...
module.exports.autocomplete = async ({ interaction }) => {
  ...
  const filtered = banners
    .filter(b => b.name.toLowerCase().includes(focused) || b.bannerId.includes(focused))
    .slice(0, 25);

  return interaction.respond(
    filtered.map(b => ({ name: b.name.substring(0, 100), value: b.bannerId }))
  );
};
...
module.exports.execute = async ({ interaction, lang }) => {
  ...
  switch (sub) {
    case "banners":    return handleBanners(interaction);
    case "pull":       return handlePull(interaction);
    case "pity":       return handlePity(interaction);
    ...
    case "gems":       return handleGems(interaction);
    default:
...
async function handlePity(interaction) {
  ...
  const [player, banners] = await Promise.all([
    GachaPlayer.findOrCreate(interaction.user.id, interaction.guildId),
    bannerId
      ? GachaBanner.find({ bannerId, active: true }).lean()
      : GachaBanner.find({ active: true }).lean(),
  ]);
