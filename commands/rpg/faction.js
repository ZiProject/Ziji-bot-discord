...
const {
  ...
  buildWarResultEmbed, buildLeaderboardEmbed,
...
const slashData = new SlashCommandBuilder()
  ...
  .addSubcommand(s => s.setName("create").setDescription("Create a new faction")
  ...
  .addSubcommand(s => s.setName("info").setDescription("View a faction")
    .addStringOption(o => o.setName("faction").setDescription("Faction name or tag").setRequired(false)))

  .addSubcommand(s => s.setName("join").setDescription("Join a faction")
  ...
  .addSubcommand(s => s.setName("members").setDescription("View faction member list")
    .addIntegerOption(o => o.setName("page").setDescription("Page number").setRequired(false)))

  .addSubcommand(s => s.setName("donate").setDescription("Donate gold to the faction treasury")
  ...
  .addSubcommand(s => s.setName("upgrade").setDescription("Upgrade a Faction Hall building")
    .addStringOption(o => o.setName("upgrade").setDescription("Upgrade to buy").setRequired(true)
  ...
  .addSubcommandGroup(g => g.setName("war").setDescription("Faction war commands")
    .addSubcommand(s => s.setName("declare").setDescription("Declare war on another faction")
      .addStringOption(o => o.setName("faction").setDescription("Target faction name or tag").setRequired(true))
      .addStringOption(o => o.setName("territory").setDescription("Territory to contest").setRequired(false)
        .addChoices(...Object.values(TERRITORIES).map(t => ({ name: `${t.emoji} ${t.name}`, value: t.id })))))
    .addSubcommand(s => s.setName("roster").setDescription("Sign up for the relay battle roster")
      .addIntegerOption(o => o.setName("position").setDescription("Battle order (1=first, 5=last)").setRequired(false).setMinValue(1).setMaxValue(5)))
  ...
  .addSubcommand(s => s.setName("rank").setDescription("Change a member's rank")
    ...
    .addStringOption(o => o.setName("rank").setDescription("New rank").setRequired(true)
      .addChoices({ name: "Officer", value: "officer" }, { name: "Elite", value: "elite" }, { name: "Member", value: "member" })))
...
module.exports.execute = async ({ interaction }) => {
  ...
  if (group === "war") {
    switch (sub) {
      case "declare":   return handleWarDeclare(interaction);
      case "roster":    return handleWarRoster(interaction);
      case "status":    return handleWarStatus(interaction);
      case "attack":    return handleWarAttack(interaction);
      case "surrender": return handleWarSurrender(interaction);
    }
  }
  ...
  switch (sub) {
    ...
    case "members":     return handleMembers(interaction);
    case "donate":      return handleDonate(interaction);
    case "hall":        return handleHall(interaction);
    case "upgrade":     return handleUpgrade(interaction);
    case "territory":   return handleTerritory(interaction);
...
async function handleInfo(interaction) {
  ...
  const memberChars = await Character.find({ userId: { $in: faction.members.map(m => m.userId) }, guildId: interaction.guildId }).lean();
...
async function handleJoin(interaction) {
  ...
  const [char, faction] = await Promise.all([
    ...
    Faction.findOne({ guildId: interaction.guildId, $or: [{ name: { $regex: `^${query}$`, $options: "i" } }, { tag: query.toUpperCase() }] }),
  ]);
...
async function handleWarDeclare(interaction) {
  ...
  const [attacker, defender] = await Promise.all([
    Faction.findById(char.factionId),
    Faction.findOne({ guildId: interaction.guildId, $or: [{ name: { $regex: `^${targetName}$`, $options:"i" } }, { tag: targetName.toUpperCase() }] }),
  ]);
  ...
  const { war, error } = await WarEngine.declareWar(attacker, defender, interaction.user.id, targetTerritory, interaction.channelId);
  if (error) return interaction.editReply({ content: error });
...
async function handleWarAttack(interaction) {
  ...
  return interaction.editReply({ content: `⚔️ ${log}`, embeds: [embed], components: _buildWarActionRows(war, char.factionId.toString()) });
}
