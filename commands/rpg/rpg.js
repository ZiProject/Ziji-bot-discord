...
const slashData = new SlashCommandBuilder()
  ...
  .addSubcommand(sub => sub
    ...
    .addStringOption(opt => opt.setName("tier").setDescription("Dungeon tier").setRequired(true)
      .addChoices(
  ...
  .addSubcommand(sub => sub
    .setName("stats")
    .setDescription("View detailed stat breakdown"));
...
module.exports.execute = async ({ interaction, lang }) => {
  ...
  switch (sub) {
    ...
    case "skills":  return handleSkills(interaction);          // [M2]
    case "skillup": return handleQuickSkillUp(interaction);    // [M2]
    case "stats":   return handleStats(interaction);            // [M2]
    default:
...
async function handleQuickSkillUp(interaction) {
  ...
  if (!skill) {
    ...
    const names = Object.values(tree).map(s => `\`${s.id}\``).join(", ");
...
async function handleStats(interaction) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      {
        name: "📈 Skills Learned",
        value: char.skills.length > 0
          ? char.skills.map(s => `\`${s.skillId}\` R${s.rank}`).join(", ")
          : "*None yet*",
        inline: false,
      },
    )
...
async function handleStart(interaction, lang) {
  ...
  const selectMenu = new StringSelectMenuBuilder()
    ...
    .addOptions(Object.values(CLASSES).map(cls => ({
      label:       cls.name,
...
async function handleProfile(interaction, lang) {
  ...
  const bar = (cur, max, len, fill, empty) => fill.repeat(Math.round(Math.max(0, Math.min(1, cur/max)) * len)) + empty.repeat(len - Math.round(Math.max(0, Math.min(1, cur/max)) * len));
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: "❤️ HP", value: `${bar(char.stats.hp, char.stats.maxHp, 10,"🟩","⬛")}\n${char.stats.hp}/${char.stats.maxHp}`, inline: true },
      { name: "💙 MP", value: `${bar(char.stats.mp, char.stats.maxMp, 10,"🟦","⬛")}\n${char.stats.mp}/${char.stats.maxMp}`, inline: true },
      { name: "⚡ Stamina", value: `${bar(char.stamina, 120, 8,"🟧","⬛")}\n${char.stamina}/120`, inline: true },
      { name: "📊 Stats", value: `⚔️ ATK: **${char.stats.atk}** | 🔮 MATK: **${char.stats.matk}**\n🛡️ DEF: **${char.stats.def}** | 🔰 MDEF: **${char.stats.mdef}**\n💨 SPD: **${char.stats.spd}** | 🎯 CRIT: **${Math.round(char.stats.crit*100)}%**`, inline: false },
      { name: "📈 XP", value: `${bar(char.xp, xpNeeded, 12,"🟨","⬛")}\n${char.xp.toLocaleString()} / ${xpNeeded.toLocaleString()}`, inline: false },
      { name: "💰 Wallet", value: `🪙 **${char.currency.gold.toLocaleString()}** gold | 💎 **${char.currency.gems}** gems`, inline: false },
...
async function handleDungeon(interaction, lang) {
  ...
  if (!char) return interaction.editReply({ content: "❌ No character. Use `/rpg start`." });
  if (char.level < config.reqLevel) return interaction.editReply({ content: `❌ Need Level **${config.reqLevel}**+ for ${config.name}.` });
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: "Info", value: `Tier **${tier}** · Floors **${config.floors}** · Stamina **${config.staminaCost}**/floor · Drops **${config.dropQuality.join(" → ")}**`, inline: false },
