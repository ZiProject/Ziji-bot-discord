...
const getProxyOptionValue = (options = [], name, type) => {
	for (const option of options) {
		if (option?.name === name && (type === undefined || option?.type === type)) {
			return option?.value;
		}

		if (Array.isArray(option?.options)) {
			const nestedValue = getProxyOptionValue(option.options, name, type);
...
const createProxyOptionsResolver = (interaction, optionData = []) => {
	const resolved = interaction.options?.resolved || {};

	const resolveValue = (name, valueType) => {
		const value = getProxyOptionValue(optionData, name, valueType);
		if (value === undefined || value === null) return null;

		switch (valueType) {
			case ApplicationCommandOptionType.User:
				return resolved?.users?.get(value) || null;
			case ApplicationCommandOptionType.Channel:
				return resolved?.channels?.get(value) || null;
			case ApplicationCommandOptionType.Role:
	...
	return {
		data: optionData,
		resolved,
		getString: (name, required = false) => resolveValue(name, ApplicationCommandOptionType.String),
		getInteger: (name, required = false) => resolveValue(name, ApplicationCommandOptionType.Integer),
		getNumber: (name, required = false) => resolveValue(name, ApplicationCommandOptionType.Number),
		getBoolean: (name, required = false) => resolveValue(name, ApplicationCommandOptionType.Boolean),
		getUser: (name, required = false) => resolveValue(name, ApplicationCommandOptionType.User),
		getChannel: (name, required = false) => resolveValue(name, ApplicationCommandOptionType.Channel),
		getRole: (name, required = false) => resolveValue(name, ApplicationCommandOptionType.Role),
		getMentionable: (name, required = false) => resolveValue(name, ApplicationCommandOptionType.Mentionable),
		getAttachment: (name, required = false) => resolveValue(name, ApplicationCommandOptionType.Attachment),
		getSubcommand: () => {
			const subcommand = optionData.find((option) => option.type === ApplicationCommandOptionType.Subcommand);
			return subcommand?.name || null;
		},
		getSubcommandGroup: () => {
			const group = optionData.find((option) => option.type === ApplicationCommandOptionType.SubcommandGroup);
...
const createProxyInteraction = (interaction, { commandName, subcommand, subcommandGroup, optionDefs = [] }) => {
	const fakeOptions = createProxyOptionsResolver(
		interaction,
...
module.exports = {
	...
	data: { name: "guildCommandManager", type: "guildCommand", enable: true },
};

module.exports.execute = async ({ action = "api", ...payload } = {}) => {
	switch (action) {
		case "buildSlashData":
			return buildSlashData(payload.record);
		case "validateCommandName":
			return validateCommandName(payload.name);
		case "validateDescription":
			return validateDescription(payload.description);
		case "validateResponsePayload":
			return validateResponsePayload(payload.type, payload.payload);
		case "loadAllToCache":
			return loadAllToCache();
		case "getGuildCommandCount":
			return getGuildCommandCount(payload.guildId);
		case "syncToCache":
			return syncToCache(payload.record);
		case "removeFromCache":
			return removeFromCache(payload.guildId, payload.name);
		case "deployGuildCommands":
			return deployGuildCommands(payload.client, payload.guildId);
		case "formatProxyTargetLabel":
			return formatProxyTargetLabel(payload.commandData);
		case "api":
		default:
			return this;
	}
