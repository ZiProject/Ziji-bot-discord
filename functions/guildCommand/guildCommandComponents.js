...
module.exports.execute = async ({ action = "api", ...payload } = {}) => {
	switch (action) {
		case "validateComponentsLayout":
			return validateComponentsLayout(payload.layout);
		case "parseComponentsLayout":
			return parseComponentsLayout(payload.raw);
		case "buildContainerFromLayout":
			return buildContainerFromLayout(payload.layout, payload.context);
		case "buildComponentsReply":
			return buildComponentsReply(payload.layout, payload.context);
		case "applyPlaceholders":
			return applyPlaceholders(payload.content, payload.context);
		case "api":
		default:
			return this;
	}
