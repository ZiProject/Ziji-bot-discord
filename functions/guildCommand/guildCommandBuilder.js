...
const buildBuilderPreview = (session, context = {}) => {
	...
	container.addActionRowComponents((row) =>
		row.addComponents(
			...
			new ButtonBuilder().setCustomId("B_guildcmd_addsection").setLabel("＋ Section").setStyle(ButtonStyle.Primary),
			new ButtonBuilder().setCustomId("B_guildcmd_addbuttons").setLabel("＋ Buttons").setStyle(ButtonStyle.Secondary),
			new ButtonBuilder().setCustomId("B_guildcmd_addmedia").setLabel("＋ Media").setStyle(ButtonStyle.Secondary),
...
module.exports.execute = async ({ action = "api", ...payload } = {}) => {
	switch (action) {
		case "getBuilderSession":
			return getBuilderSession(payload.userId, payload.guildId);
		case "setBuilderSession":
			return setBuilderSession(payload.userId, payload.guildId, payload.session);
		case "clearBuilderSession":
			return clearBuilderSession(payload.userId, payload.guildId);
		case "startBuilderSession":
			return startBuilderSession(payload);
		case "buildBuilderPreview":
			return buildBuilderPreview(payload.session, payload.context);
		case "api":
		default:
			return this;
	}
