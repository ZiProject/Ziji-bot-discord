...
module.exports.execute = async ({ interaction, lang }) => {
	...
	const memberIdTarget = channel.permissionOverwrites.cache.find(
		(p) => p.type === 1 && p.id !== interaction.guild.roles.everyone.id && p.id !== staffRoleId,
	)?.id;
	let ticketCreator = null;
	if (memberIdTarget) {
		try {
			ticketCreator = await interaction.guild.members.fetch(memberIdTarget);
		} catch {}
	}
	...
	if (process.env.API_URL) {
		...
		try {
			...
			await ticketCreator.send({
				embeds: [dmEmbed],
				components: [{ type: 1, components: [{ type: 2, label: "📂 Xem Transcript", style: 5, url }] }],
			});
			dmSuccess = true;
		} catch (err) {
		...
		const attachment = new AttachmentBuilder(buffer, {
			name: `transcript-${channel.name}-${Math.random().toString(36).substring(2, 12)}.html`,
		});
		if (ticketCreator) {
			try {
				const dmEmbed = {
			...
			logChannel.send({
				...
				attachments:
					process.env.API_URL ?
						[]
					:	[
							new AttachmentBuilder(Buffer.from(template, "utf-8"), {
								name: `transcript-${channel.name}-${Math.random().toString(36).substring(2, 12)}.html`,
							}),
						],
			});
