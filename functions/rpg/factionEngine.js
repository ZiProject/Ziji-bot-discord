...
const { Character, Faction } = useHooks.get("db");
...
class FactionEngine {
	...
	static async donate(faction, char, amount) {
		...
		if (char.currency.gold < amount) {
			return {
				ok: false,
				error: `❌ Not enough gold (have ${char.currency.gold.toLocaleString()}, donating ${amount.toLocaleString()}).`,
			};
		}
	...
	static async distributeDailyIncome(faction) {
		...
		for (const terr of faction.territories) {
			...
			switch (def.bonusType) {
				case "gold_income":
					goldIncome += def.bonusValue;
					break;
				case "gem_income":
					gemIncome += def.bonusValue;
					break;
				case "token_income":
					tokenIncome += def.bonusValue;
					break;
			}
		}
