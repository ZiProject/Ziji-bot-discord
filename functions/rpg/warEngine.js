...
class WarEngine {
	...
	static async resolveRelayRound(war, roundNumber) {
		...
		const [attackerChar, defenderChar] = await Promise.all([
			attackerPlayer.userId.startsWith("bot_") ?
				null
			:	Character.findOne({ userId: attackerPlayer.userId, guildId: war.guildId }),
			defenderPlayer.userId.startsWith("bot_") ?
				null
			:	Character.findOne({ userId: defenderPlayer.userId, guildId: war.guildId }),
		]);

		const atkStats =
			attackerChar ? WarEngine._buildWarSnapshot(attackerChar, war, attackerSide) : WarEngine._buildBotSnapshot(attackerPlayer);

		const defStats =
			defenderChar ? WarEngine._buildWarSnapshot(defenderChar, war, defenderSide) : WarEngine._buildBotSnapshot(defenderPlayer);
	...
	static async _resolveWar(war) {
		...
		const winnerFaction =
			winner === "alpha" ? alphaFaction
			: winner === "beta" ? betaFaction
			: null;
		const loserFaction =
			winner === "alpha" ? betaFaction
			: winner === "beta" ? alphaFaction
			: null;
		...
		if (winner === "draw") {
			...
			for (const [uid, dmg] of war.siegeDamageLog.entries()) {
				...
				if (side === winnerSide && dmg > topDamage) {
					topDamage = dmg;
					mvpUserId = uid;
				}
			}
