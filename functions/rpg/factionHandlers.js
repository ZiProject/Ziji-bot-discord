...
const { buildWarStatusEmbed, buildWarResultEmbed, buildMemberListEmbed } = require("./factionUI");
...
module.exports.warSiege = {
	...
	execute: async (interaction) => {
		...
		const [war, char] = await Promise.all([FactionWar.findById(warId), _loadChar(interaction.user.id, interaction.guildId)]);
		if (!war) return interaction.followUp({ content: "❌ War not found.", ephemeral: true });
		if (!char) return interaction.followUp({ content: "❌ No character.", ephemeral: true });
...
function _buildWarActionRows(war, factionId) {
	...
	if (war.phase === "relay") {
		rows.push(
			new ActionRowBuilder().addComponents(
				new ButtonBuilder()
					.setCustomId(`B_WAR_RELAY_${war._id}`)
					.setLabel("⚔️ Next Round (Officers only)")
	...
	if (war.phase === "siege") {
		...
		const playerSide =
			war.alpha?.roster?.some((p) => p.userId === factionId) ? "alpha"
			: war.beta?.roster?.some((p) => p.userId === factionId) ? "beta"
			: null;
		// Rough check — proper side check happens server-side in WarEngine
		rows.push(
			new ActionRowBuilder().addComponents(
				new ButtonBuilder().setCustomId(`B_WAR_SIEGE_${war._id}`).setLabel("🏹 Attack Siege (5 ⚡)").setStyle(ButtonStyle.Danger),
				new ButtonBuilder().setCustomId(`B_WAR_STATUS_${war._id}`).setLabel("🔄 Refresh").setStyle(ButtonStyle.Secondary),
			),
		);
	}
		...
		try {
			...
			for (const war of activeWars) {
				...
				if (war.phase === "preparation" && war.preparationEnds && now >= new Date(war.preparationEnds)) {
					...
					if (ok) {
						await WarScheduler._announcePhase(
							client,
							war,
							"⚔️ **Relay Phase has started!** Leaders can now advance rounds with the button.",
						);
					}
				}
